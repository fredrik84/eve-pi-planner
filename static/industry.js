...
async function onIndustryTabOpen() {
  ...
  ppCloseTransientMenus('.ind-action-menu');
...
function _indPaintStatus(d, opts) {
  ...
  const rxIssues = d.reaction_handoff
    ? [...(d.reaction_handoff.shortfalls || []), ...(d.reaction_handoff.conflicts || [])] : [];
  if (!local && rxIssues.length) {
    const ids = rxIssues.map(x => x.order_id).sort((a, b) => a - b).join(',');
    ...
    if (sessionStorage.getItem('ind-rx-shortfall-shown') !== key) {
      ...
      toast(rxIssues[0].detail || 'Ready Manufacturing reactions need attention in the Reactions queue.', 'warning');
    }
  }
...
function _indStatusHeadline(d, viewOrderId) {
  ...
  return sim
    ...
    + `<div class="ind-action-menu-pop"><button class="ind-secondary-btn" onclick="ppCloseTransientMenus('.ind-action-menu');indOpenSetup()">Job slots &amp; setup</button>`
    + (!viewOrderId && (_indOrders || []).length > 1
        ? `<button class="ind-secondary-btn" onclick="ppCloseTransientMenus('.ind-action-menu');indOpenOrder()">Reorder builds</button>` : '')
    + `<button class="ind-secondary-btn" onclick="ppCloseTransientMenus('.ind-action-menu');indRefreshJobs()" title="Pull job status from EVE and re-plan">Refresh ESI jobs</button></div></details></div>`
