...
Run: python3 tests/test_industry.py
"""
import json
import math
import inspect
...
def test_phase5_recovers_and_links_shared_production_work():
    """Phase 5 adds orchestration around the two engines without merging allocation math."""
    print("test_phase5_recovers_and_links_shared_production_work")
    import inspect
    import app.reactions.orders as reaction_orders
    import app.reactions.jobs as reaction_jobs
    import app.industry.jobs as industry_jobs
    ...
    check("both ESI refresh paths trigger the same reaction recovery",
          "retry_automatic_orders" in inspect.getsource(reaction_jobs.refresh_industry_jobs)
          and "retry_automatic_orders" in inspect.getsource(industry_jobs.refresh_manufacturing_jobs))
    owners = inspect.getsource(reaction_orders._attach_manufacturing_sources)
    check("reaction orders expose every Manufacturing owner under the account boundary",
          "pp_reaction_order_sources" in owners and "i.context_id=s.context_id" in owners
          and '"label"' in owners and '"runs"' in owners)
    js = _industry_js()
    rx = open("static/reactions.js", encoding="utf-8").read()
    router = open("static/app.js", encoding="utf-8").read()
    check("Manufacturing reaction cards open the Reactions order",
          "_indReactionOrderLinks" in js and "_indGoToReactionOrder" in js)
    check("Reactions links every owner back to Manufacturing",
          "_rxManufacturingSourcesHtml" in rx and "_rxGoToManufacturingOrder" in rx)
    check("blocked work is grouped into one attention surface",
          "_rxAttentionHtml" in rx and "need attention" in rx)
    check("reaction orders have private deep links like Manufacturing orders",
          "_rxOpenOrderLink" in rx and "reactions:" in router and "noteRecord('reactions'" in rx)
...
def main():
    ...
    test_phase5_recovers_and_links_shared_production_work()
