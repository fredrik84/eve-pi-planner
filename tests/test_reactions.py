...
container via a fabricated pp_sessions cookie (same pattern as tests/test_alerts.py's live
...
Usage:
    python tests/test_reactions.py [--url http://localhost:8000]
"""
import argparse
import json
import math
...
def test_order_owned_slots_have_compact_source_keys_and_a_legend() -> bool:
    ...
    ok &= check("rx-slot-planned-time" in js and "left: -5px; right: auto; transform: none" in open(
        "static/style-layout-admin.css").read(),
                "planned runtime is left-aligned away from the top-right Installed and Clear controls")
...
def test_reaction_transactions_are_postgres_compatible() -> bool:
    """Production is PostgreSQL; SQLite-only BEGIN IMMEDIATE broke the whole Reactions page."""
