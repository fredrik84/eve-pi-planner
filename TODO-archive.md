...
| 20 | Clear all no longer strands a customer order: order rows are cleared AND the order's `assigned_runs` handed back (top row per chain, clamped at 0), with already-stranded orders repaired on the next assign. `tests/test_clear_all_orders.py` | 08-07 |
...
| 28 | One run count per product across EVERY character (`level_product_runs`, `reactions_level_runs`): Carbon Fiber at 125/90/90/75 on four characters becomes 125 everywhere in 12 jobs instead of 15, chosen by spread → slots → surplus and never running longer than the stage's longest job already planned. `tests/test_level_runs.py` | 08-08 |
...
2. **`bp._manual_enabled` monkeypatching went inert** — three checks in
   `tests/test_manual_blueprints.py` went red, which is the good outcome. The helper now patches every
   module binding the name, discovered by walking `sys.modules` rather than naming today's two.
3. **`G._default_system_on` / `_fallback_build_system`** — same shape in `tests/test_cost_basis.py`; all
   three of its scenarios would have collapsed into one. Retargeted at `graph.options`.

Also: `inspect.getsource(<module>)` returns only `__init__.py` for a package, so `tests/test_industry.py`'s
...
`tests/test_planetdb_guard.py` fails on any unscoped global delete that is not admin-gated, so the shape
...
| Alert-engine rename | **Done** (2026-07-30). `app/colony_alerts.py` → `app/alerts.py`, `compute_colony_alerts()` → `compute_alerts()`, `tests/test_colony_alerts.py` → `tests/test_alerts.py`. Pure rename, zero behaviour change; `tests/test_alerts.py` passes in-container incl. the live `/api/dashboard` layer. |
